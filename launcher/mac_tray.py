...
class MacTrayObject(NSObject):
    ...
    def applicationDidFinishLaunching_(self, notification):
        if lang_code == "zh_CN":
            ...
            self.menu_options = ['Config',
                                 'Enable Global GAEProxy',
                                 'Enable Auto GAEProxy',
                                 'Disable GAEProxy',
                                 'Reload GAEProxy',
                                 'Quit']
        self.setupUI()
    ...
    def setupUI(self):
        ...
        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[0], 'config:', '')
        self.menu.addItem_(menuitem)

        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[1], 'enableGlobalProxy:', '')
        if proxyState == 'gae':
            menuitem.setState_(NSOnState)
        self.menu.addItem_(menuitem)

        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[2], 'enableAutoProxy:', '')
        if proxyState == 'pac':
            menuitem.setState_(NSOnState)
        self.menu.addItem_(menuitem)

        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[3], 'disableProxy:', '')
        if proxyState == 'disable':
            menuitem.setState_(NSOnState)
        self.menu.addItem_(menuitem)

        # Reset Menu Item
        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[4], 'resetGoagent:', '')
        self.menu.addItem_(menuitem)
        # Default event
        menuitem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(self.menu_options[5], 'windowWillClose:', '')
        self.menu.addItem_(menuitem)
    ...
    def updateStatusBarMenu(self):
        globalGaeProxyMenuItem  = self.menu.itemWithTitle_(self.menu_options[1])
        autoGaeProxyMenuItem    = self.menu.itemWithTitle_(self.menu_options[2])
        disableGaeProxyMenuItem = self.menu.itemWithTitle_(self.menu_options[3])
