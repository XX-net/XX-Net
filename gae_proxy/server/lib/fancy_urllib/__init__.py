...
def _create_fancy_connection(tunnel_host=None, key_file=None,
    ...
    class PresetProxyHTTPSConnection(httplib.HTTPSConnection):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if tunnel_host:
                logging.debug(
                    "Creating preset proxy https conn: %s", tunnel_host)
...
# Here to end of _create_connection copied wholesale from Python 2.6"s
# socket.py
...
class FancyProxyHandler(urllib2.ProxyHandler):
    ...
    def proxy_open(self, req, proxy, type):
        ...
        if user and password:
            user_pass = "%s:%s" % (urllib2.unquote(
                user), urllib2.unquote(password))
