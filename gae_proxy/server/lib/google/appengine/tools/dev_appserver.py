...
class MatcherDispatcher(URLDispatcher):
    ...
    def Dispatch(self,
        ...
        for matcher in self._url_matchers:
            dispatcher, matched_path, requires_login, admin_only, auth_fail_action = matcher.Match(
...
def SetupSharedModules(module_dict):
    ...
    for module_name, module in module_dict.iteritems():
        ...
        shared_prefix = ModuleNameHasPrefix(
            module_name, SHARED_MODULE_PREFIXES)
        banned_prefix = ModuleNameHasPrefix(
            module_name, NOT_SHARED_MODULE_PREFIXES)
...
def LoadTargetModule(handler_path,
    ...
    if script_module is not None and ModuleHasValidMainFunction(script_module):
        ...
        logging.debug('Reusing main() function of module "%s"',
                      module_fullname)
    else:
        ...
        try:
            ...
            if not os.path.isfile(cgi_path):
                ...
                try:
                    ...
                    try:
                        module_code = compile(
                            source_file.read(), cgi_path, 'exec')
    ...
    try:
        ...
        try:
            ...
            if handler_path and config and config.runtime == 'python27':
                reset_modules = exec_py27_handler(
                    config, handler_path, cgi_path, hook)
            else:
                reset_modules = exec_script(
                    config, handler_path, cgi_path, hook)
        except SystemExit, e:
...
class StaticFileConfigMatcher(object):
    ...
    def __init__(self,
        ...
        if default_expiration is not None:
            self._default_expiration = appinfo.ParseExpiration(
                default_expiration)
        else:
...
class FileDispatcher(URLDispatcher):
    ...
    def Dispatch(self,
        ...
        content_type = self._static_file_config_matcher.GetMimeType(
            request.path)
        static_file = self._static_file_config_matcher.IsStaticFile(
            request.path)
        expiration = self._static_file_config_matcher.GetExpiration(
            request.path)
        current_etag = self.CreateEtag(data)
        if_match_etag = request.headers.get('if-match', None)
        if_none_match_etag = request.headers.get(
            'if-none-match', '').split(',')
...
def CreateRequestHandler(root_path,
    ...
    class DevAppServerRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
        ...
        def _HandleRequest(self):
            ...
            full_url = GetFullURL(
                server_name, self.server.server_port, self.path)
            ...
            try:
                ...
                email_addr, admin, user_id = dev_appserver_login.GetUserInfo(
                    cookies)
                ...
                response = RewriteResponse(
                    outfile, self.rewriter_chain, self.headers)
                ...
                logging.error(msg)
                self.send_response(
                    httplib.INTERNAL_SERVER_ERROR, 'Compile error')
                self.wfile.write(
...
def CreateURLMatcherFromMaps(config,
    ...
    for url_map in url_map_list:
        ...
        if handler_type == appinfo.HANDLER_SCRIPT:
            ...
            raise InvalidAppConfigError(
                'Unknown handler type "%s"' % handler_type)
...
def SetupStubs(app_id, **config):
    ...
    if clear_prospective_search and prospective_search_path:
        ...
        if os.path.lexists(prospective_search_path):
            logging.info('Attempting to remove file at %s',
...
def CreateServer(root_path,
    ...
    if multiprocess.Enabled():
        server = HttpServerWithMultiProcess(
            (serve_address, port), handler_class)
    else:
