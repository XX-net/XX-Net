    ...
    def PerformWork(self, thread_pool):
        ...
        try:
            ...
            try:
                ...
                status = adaptive_thread_pool.WorkItem.RETRY
                logger.exception(
                    'Retrying on non-fatal datastore error: %s', e)
                ...
                if IsURLErrorFatal(e):
                    ...
                    status = adaptive_thread_pool.WorkItem.RETRY
                    logger.exception(
                        'Retrying on non-fatal URL error: %s', e.reason)
...
def IncrementId(high_id_key):
    ...
    """
    unused_start, end = datastore.AllocateIds(
        high_id_key, max=high_id_key.id())
...
class RequestManager(object):
    ...
    def __init__(self,
        ...
        logger.debug('Bulkloader using app_id: %s',
    ...
    def IncrementId(self, ancestor_path, kind, high_id):
        ...
        if self.dry_run:
            return
        high_id_key = datastore.Key.from_path(
            *(ancestor_path + [kind, high_id]))
        ...
        if not loader:
            try:
                ...
                raise ConfigurationError(
                    'No Loader defined for kind %s.' % self.kind)
        entities = []
    ...
    def GetMapper(self, kind):
        ...
        if not self.mapper:
            try:
                ...
                raise ConfigurationError(
                    'No Mapper defined for kind %s.' % kind)
        return self.mapper
...
class _ThreadBase(threading.Thread):
    ...
    * Friendly names for presenting to users.
    ...
    def run(self):
        ...
        logger.debug('[%s] %s: started', self.getName(),
        ...
        try:
            ...
            self.SetError()
            logger.exception('[%s] %s:', self.getName(),
                             self.__class__.__name__)

        logger.debug('[%s] %s: exiting', self.getName(),
                     self.__class__.__name__)
...
class ResultDatabase(_Database):
    ...
    def _StoreEntity(self, entity_id, entity):
        ...
        if self.sort_key_from_entity:
            sort_key = self.sort_key_from_entity(
                datastore.Entity._FromPb(entity))
        else:
            sort_key = ''
        value = entity.Encode()
        self.insert_cursor.execute(
...
class _ProgressDatabase(_Database):
    ...
    def UseProgressData(self):
        ...
        if row is None:
            raise ResumeError(
                'Cannot retrieve progress information from database.')
...
class RestoreLoader(Loader):
    ...
    def initialize(self, filename, loader_opts):
        ...
        self.high_id_table = self._find_high_id(
        ...
        while not thread_local.shut_down:
            ...
            if self.data_source_thread.isAlive():
                ...
                for thread in list(thread_pool.Threads()) + [self.progress_thread]:
                    if not thread.isAlive():
                        logger.info('Unexpected thread death: %s',
                                    thread.getName())
                        thread_local.shut_down = True
                        self.error = True
                        break
    ...
    def RunPostAuthentication(self):
        ...
        for ancestor_path, kind_map in high_id_table.iteritems():
            for kind, high_id in kind_map.iteritems():
                self.request_manager.IncrementId(
                    list(ancestor_path), kind, high_id)
        return [self.kind]
...
def _PerformBulkload(arg_dict,
    ...
    if not app_id:
        if dry_run:
            ...
            raise ConfigurationError(
                'Must sepcify application ID in dry run mode.')
    ...
    if upload or restore:
        ...
        result_db = ResultDatabase(
            result_db_filename, signature, exporter=exporter)
        try:
            ...
            try:
                ...
                logger.error(
                    'Kind stats are generated periodically by the appserver')
