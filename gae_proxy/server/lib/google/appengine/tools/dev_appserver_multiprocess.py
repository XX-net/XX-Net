...
class ChildProcess(object):
    ...
    def __str__(self):
        if self.api_server:
            ...
            string = 'Backend Instance: %s.%d' % (
                self.backend_id, self.instance_id)
    ...
    def Start(self, argv, api_port):
        ...
        if self.instance_id is not None:
            self.SetFlag('--multiprocess_backend_instance_id',
    ...
    def SetFlag(self, flag, short_flag=None, value=None):
        ...
        """
        self.RemoveFlag(flag, short_flag=short_flag,
...
class DevProcess(object):
    ...
    def Init(self, appinfo, backends, options):
        ...
        if not self.backends:
            raise Error(
                'Entering multiprocess mode with no backends configured.')
        ...
        if ARG_MULTIPROCESS_BACKEND_ID in options:
            ...
            if ARG_MULTIPROCESS_BACKEND_INSTANCE_ID in options:
                ...
                self.instance_id = int(
                    options[ARG_MULTIPROCESS_BACKEND_INSTANCE_ID])
        ...
        if ARG_MULTIPROCESS_MIN_PORT in options:
            self.multiprocess_min_port = int(
                options[ARG_MULTIPROCESS_MIN_PORT])
    ...
    def StartChildren(self, argv):
        ...
        self.child_api_server = ChildProcess(
            self.host, next_port, api_server=True)
