...
class UpdateCheck(object):
        ...
        try:
            ...
            try:
                ...
                if this_release > advertised_release:
                    logging.info(
                        'This SDK release is newer than the advertised release.')
                    return
    ...
    def _WriteNagFile(self, nag):
        ...
        try:
            ...
            logging.error(
                'Could not write nag file to %s. Error: %s', nagfilename, e)
...
class LogsRequester(object):
        ...
        try:
            while True:
                try:
                    ...
                    StatusUpdate(
                        'Keyboard interrupt; saving data downloaded so far.')
                    break
    ...
    def RequestLogLines(self, tf, offset):
        ...
        response = self.rpcserver.Send(
            '/api/request_logs', payload=None, **kwds)
        response = response.replace('\r', '\0')
        lines = response.splitlines()
        logging.info('Received %d bytes, %d records.',
                     len(response), len(lines))
        offset = None
...
def FindSentinel(filename, blocksize=2**16):
    ...
    try:
        ...
        StatusUpdate('Append mode disabled: can\'t read %r: %s.' %
                     (filename, err))
        return None
    try:
        fp.seek(0, 2)
...
class UploadBatcher(object):
    ...
    def SendBatch(self):
        ...
        for path, payload, mime_type in self.batch:
            while boundary in payload:
                ...
                assert len(
                    boundary) < 80, 'Unexpected error, please try again.'
            part = '\n'.join(['',
    ...
    def Flush(self):
        ...
        try:
            ...
            logging.info(
                'Old server detected; turning off %s batching.', self.what)
            self.batching = False
...
def DoDownloadApp(rpcserver, out_dir, app_id, app_version):
    ...
    for line in file_lines:
        ...
        try:
            try:
                ...
                logging.error('Couldn\'t write to file "%s": %s',
                              full_path, exc)
                num_errors += 1
                continue
        finally:
    ...
    if num_errors > 0:
        logging.error(
            'Number of errors: %d.  See output for details.', num_errors)
...
class AppVersionUpload(object):
    ...
    def DoUpload(self, paths, max_size, openfunc):
        ...
        try:
            ...
            if missing_files:
                StatusUpdate('Uploading %d files and blobs.' %
                             len(missing_files))
                num_files = 0
        ...
        for entry in sorted(entries):
            ...
            if os.path.isfile(fullname):
                if skip_files.match(name):
                    logging.info(
                        'Ignoring file \'%s\': File matches ignore regex.', name)
                else:
                    yield name
...
class AppCfgApp(object):
    ...
    def _MakeSpecificParser(self, action):
        ...
        parser.set_description('%s\n%s' % (
            action.short_desc, action.long_desc))
    ...
    def DownloadApp(self):
        ...
        if app_id is None:
            self.parser.error(
                'You must specify an app ID via -A or --application.')
    ...
    def Update(self):
        ...
        if index_defs:
            index_upload = IndexDefinitionUpload(
                rpcserver, appyaml, index_defs)
            try:
    ...
    def BackendsYamlCheck(self, appyaml, backend=None):
        ...
        for backend_entry in appyaml.backends:
            ...
            if entry.name in backends:
                self.parser.error(
                    'Duplicate entry for backend: %s.' % entry.name)
            else:
    ...
    def BackendsList(self):
        ...
        appyaml = self._ParseAppYaml(self.basepath)
        rpcserver = self._GetRpcServer()
        response = rpcserver.Send(
            '/api/backends/list', app_id=appyaml.application)
