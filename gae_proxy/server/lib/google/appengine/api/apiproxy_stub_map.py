    ...
    def MakeSyncCall(self, service, call, request, response):
        ...
        if hasattr(stub, 'CreateRPC'):
            ...
            try:
                ...
                self.__postcall_hooks.Call(
                    service, call, request, response, rpc, err)
                raise
            else:
                self.__postcall_hooks.Call(
            ...
            try:
                returned_response = stub.MakeSyncCall(
                    service, call, request, response)
            except Exception, err:
                self.__postcall_hooks.Call(
                    service, call, request, response, None, err)
                raise
            else:
...
class UserRPC(object):
    ...
    def wait_any(cls, rpcs):
        ...
        """
        assert iter(
            rpcs) is not rpcs, 'rpcs must be a collection, not an iterator'
