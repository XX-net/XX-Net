...
class User_config(object):
    ...
    def load(self):
        ...
        CONFIG_USER_FILENAME = os.path.abspath(os.path.join(
            root_path, 'data', 'gae_proxy', 'config.ini'))
        ...
        try:
            if os.path.isfile(DEFAULT_CONFIG_FILENAME):
                ...
                self.ip_connect_interval = self.DEFAULT_CONFIG.getint(
                    'google_ip', 'ip_connect_interval')
            else:
                return
            ...
            try:
                self.user_special.ip_connect_interval = config.CONFIG.getint(
                    'google_ip', 'ip_connect_interval')
            except:
                pass
            ...
            try:
                self.user_special.use_ipv6 = config.CONFIG.getint(
                    'google_ip', 'use_ipv6')
            except:
                pass

            try:
                self.user_special.connect_interval = config.CONFIG.getint(
    ...
    def save(self):
        ...
        try:
            ...
            xlog.warn("launcher.config save user config fail:%s",
...
class ControlHandler():
    ...
    def do_GET(self):
        ...
        if path == "/log":
            ...
            xlog.debug('GAEProxy Web_control %s %s %s ',
        ...
        if path == '/deploy':
            ...
            file_path = os.path.abspath(os.path.join(
                web_ui_path, '/'.join(path.split('/')[1:])))
            if not os.path.isfile(file_path):
                self.wfile.write(b'HTTP/1.1 404 Not Found\r\n\r\n')
                xlog.warn('%s %s %s wizard file %s not found',
                          self.address_string(), self.command, self.path, file_path)
                return
            ...
            xlog.warn('Control Req %s %s %s ',
                      self.address_string(), self.command, self.path)

        # check for '..', which will leak file
        if re.search(r'(\.{2})', self.path) is not None:
            self.wfile.write(b'HTTP/1.1 404\r\n\r\n')
            xlog.warn('%s %s %s haking', self.address_string(),
    ...
    def req_log_handler(self):
        ...
        if cmd == "set_buffer_size":
            ...
            xlog.error('PAC %s %s %s ', self.address_string(),
                       self.command, self.path)
    ...
    def get_launcher_version(self):
        data_path = os.path.abspath(os.path.join(
            root_path, 'data', 'launcher', 'config.yaml'))
        try:
    ...
    def get_os_language(self):
        ...
        if sys.platform == "darwin":
            try:
                oot = os.pipe()
                p = subprocess.Popen(
                    ["/usr/bin/defaults", 'read', 'NSGlobalDomain', 'AppleLanguages'], stdout=oot[1])
                p.communicate()
    ...
    def req_config_handler(self):
        ...
        try:
            if reqs['cmd'] == ['get_config']:
                ...
                user_config.user_special.password = self.postvars[
                    'password'][0]
                ...
                user_config.user_special.ip_connect_interval = int(
                    self.postvars['ip_connect_interval'][0])
                user_config.user_special.use_ipv6 = int(
                    self.postvars['use_ipv6'][0])
                user_config.user_special.connect_interval = int(
                    self.postvars['connect_interval'][0])
                user_config.save()
    ...
    def req_deploy_handler(self):
        ...
        log_path = os.path.abspath(os.path.join(
            current_path, os.pardir, "server", 'upload.log'))
        ...
        if reqs['cmd'] == ['deploy']:
            ...
            if deploy_proc and deploy_proc.poll() == None:
                ...
                try:
                    ...
                    script_path = os.path.abspath(os.path.join(
                        current_path, os.pardir, "server", 'uploader.py'))
                    ...
                    deploy_proc = subprocess.Popen(
                        [sys.executable, script_path, appid, email, passwd, rc4_passwd])
            ...
            if deploy_proc and deploy_proc.poll() == None:
                ...
                data = '{"res":"deploy is not running", "time":"%s"}' % (
                    time_now)
            ...
            data = json.dumps(
                {'status': status, 'log': content, 'time': time_now})
