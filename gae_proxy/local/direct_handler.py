...
def handler(method, host, url, headers, body, wfile):
    ...
    while True:
        ...
        try:
            ...
            if response:
                if response.status > 400:
                    ...
                    if "gws" not in server_type:
                        xlog.warn("IP:%s not support GAE, server type:%s status:%d",
            ...
            errors.append(e)
            xlog.exception(
                'direct_handler.handler %r %s %s , retry...', e, host, url)
    ...
    try:
        ...
        try:
            ...
            xlog.warn(
                "direct_handler.handler send response fail. t:%d e:%r %s%s", wait_time, e, host, url)

        if method == 'HEAD' or response.status in (204, 304):
            xlog.info("DIRECT t:%d %d %s %s", (time.time() -
                                               time_request) * 1000, response.status, host, url)
        ...
        if 'Transfer-Encoding' in response_headers:
            ...
            while True:
                ...
                if send_to_browser:
                    try:
                        ...
                        send_to_browser = False
                        xlog.warn("direct_handler.handler send Transfer-Encoding t:%d e:%r %s/%s",
            ...
            response.close()
            xlog.info("DIRECT chucked t:%d s:%d %d %s %s", (time.time(
            ) - time_request) * 1000, length, response.status, host, url)
            return
        ...
        while True:
            if start > end:
                https_manager.save_ssl_connection_for_reuse(
                    response.ssl_sock, host)
                xlog.info("DIRECT t:%d s:%d %d %s %s", (time.time() -
                                                        time_request) * 1000, length, response.status, host, url)
                return
            ...
            if not data:
                if time.time() - time_last_read > 20:
                    response.close()
                    xlog.warn("read timeout t:%d len:%d left:%d %s %s", (time.time(
                    ) - time_request) * 1000, length, (end - start), host, url)
                    return
                else:
                    time.sleep(0.1)
                    continue
            ...
            if send_to_browser:
                try:
                    ...
                    if e_b[0] in (errno.ECONNABORTED, errno.EPIPE, errno.ECONNRESET) or 'bad write retry' in repr(e_b):
                        xlog.warn(
                            'direct_handler send to browser return %r %s %r', e_b, host, url)
                    else:
                        xlog.warn(
                            'direct_handler send to browser return %r %s %r', e_b, host, url)
                    send_to_browser = False
        ...
        if e[0] in (errno.ECONNABORTED, errno.EPIPE) or 'bad write retry' in repr(e):
            xlog.exception("direct_handler err:%r %s %s time:%d",
                           e, host, url, time_cost)
        else:
