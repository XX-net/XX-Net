...
class CertUtil(object):
        ...
        with open(certfile, 'rb') as fp:
            ...
            if not ret and __name__ != "__main__":
                ...
                CertUtil.win32_notify(
                    msg=u'已经导入GoAgent证书，请重启浏览器.', title=u'Restart browser need.')
    ...
    def import_linux_firefox_ca(common_name, ca_file):
        ...
        cmd_line = 'certutil -L -d %s |grep "GoAgent" &&certutil -d %s -D -n "%s" ' % (
    ...
    def import_debian_ca(common_name, ca_file):
        ...
        def get_debian_ca_sha1(nss_path):
            # TODO: here should be GoAgent - XX-Net
        ...
        cmd_line = 'certutil -L -d sql:$HOME/.pki/nssdb |grep "GoAgent" && certutil -d sql:$HOME/.pki/nssdb -D -n "%s" ' % (
            common_name)
        os.system(cmd_line)
    ...
    def import_mac_ca(common_name, certfile):
        ...
        import_command = 'security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ../../data/gae_proxy/CA.crt'
        if exist_ca_sha1:
