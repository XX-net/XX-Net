...
class GAEProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def forward_local(self):
        html = gae_handler.generate_message_html(
            'Browser pass local request to proxy', u'您的浏览器把本地请求转发到代理上。<br>请在浏览器中设置：访问本地，不经过代理。<br><a href="https://github.com/XX-net/XX-Net/wiki/Browser-pass-localhost-request-to-proxy">帮助</a>')
    ...
    def do_AGENT(self):
        ...
        gae_handler.handler(self.command, self.path,
                            request_headers, payload, self.wfile)
    ...
    def do_CONNECT_FWD(self):
        ...
        if host == "appengine.google.com" or host == "www.google.com":
            # gae_proxy upload to appengine is slow, it need more 'fresh'
            # connection.
            connected_in_s = 5
        else:
        ...
        try:
            ...
            xlog.exception(
                'do_CONNECT_FWD (%r, %r) Exception:%s', host, port, e)
            self.connection.close()
            return

        remote = forwork_manager.create_connection(
            host=host, port=port, sock_life=connected_in_s)
        if remote is None:
            self.connection.close()
            xlog.warn('FWD %s %s:%d create_connection fail',
                      self.command, host, port)
            return
        ...
        # reset timeout default to avoid long http upload failure, but it will
        # delay timeout retry :(
        remote.settimeout(None)

        forwork_manager.forward_socket(
            self.connection, remote, bufsize=self.bufsize)
        xlog.debug('FWD %s %s:%d with closed', self.command, host, port)

    def do_CONNECT_AGENT(self):
        """deploy fake cert to client"""
        # GAE supports the following HTTP methods: GET, POST, HEAD, PUT,
        # DELETE, and PATCH
        host, _, port = self.path.rpartition(':')
        port = int(port)
        ...
        try:
            ...
            xlog.info(
                'ssl error: %s, create full domain cert for host:%s', e, host)
        ...
        if self.command not in self.gae_support_methods:
            if host.endswith(".google.com") or host.endswith(config.HOSTS_FWD_ENDSWITH) or host.endswith(config.HOSTS_GAE_ENDSWITH):
                ...
                xlog.warn(
                    "Method %s not support in GAE, Redirect to FWD for %s", self.command, self.path)
                return self.wfile.write(('HTTP/1.1 301\r\nLocation: %s\r\n\r\n' % self.path).encode())
            else:
    ...
    def do_CONNECT_DIRECT(self):
        ...
        try:
            ...
            request_headers = dict((k.title(), v)
            ...
            if 'Content-Length' in request_headers:
                try:
                    ...
                    xlog.error(
                        'handle_method_urlfetch read payload failed:%s', e)
                    return

            direct_handler.handler(
                self.command, host, path, request_headers, payload, self.wfile)
