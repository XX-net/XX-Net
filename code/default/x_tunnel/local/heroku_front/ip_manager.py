...
class IpManager():
    ...
    def load_config(self):
        ...
        if self.use_ipv6:
            good_ip_file_name = "heroku_ipv6.txt"
            default_good_ip_file_name = "good_ipv6.txt"
            self.max_scan_ip_thread_num = 0
            self.auto_adjust_scan_ip_thread_num = 0
        else:
            good_ip_file_name = "heroku_ip.txt"
            default_good_ip_file_name = "good_ip.txt"
            self.max_scan_ip_thread_num = config.CONFIG.getint("ip_manager", "max_scan_ip_thread_num") #50
            self.auto_adjust_scan_ip_thread_num = config.CONFIG.getint("ip_manager", "auto_adjust_scan_ip_thread_num")
        ...
        self.max_links_per_ip = config.CONFIG.getint("ip_manager", "max_links_per_ip")
        self.max_good_ip_num = config.CONFIG.getint("ip_manager", "max_good_ip_num") #3000  # stop scan ip when enough
        self.ip_connect_interval = config.CONFIG.getint("ip_manager", "ip_connect_interval") #5,10
    ...
    def load_ip(self):
        ...
        xlog.info("load ip_list num:%d, gws num:%d", len(self.ip_dict), len(self.gws_ip_list))
    ...
    def add_ip(self, ip, handshake_time, domain=None, server='', fail_times=0, down_fail=0):
        ...
        if self.use_ipv6 and ":" not in ip:
...
if __name__ == "__main__":
    try:
        ip_manager.scan_all_exist_ip()
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        xlog.info("Keyboard Interrupt, exit.")
